...
That's it. Patly handles everything else — signing, fee, relaying.
...
When you call `patly.redeem()`, the library builds a **Safe multisend transaction** containing two calls:

1. `redeemPositions()` — claims your winning USDC
2. `USDC.transfer($0.01 → Patly)` — the fee, paid from your winnings

Both happen atomically in a single gasless transaction. If the redemption fails, the fee is never paid. No separate USDC balance is required.
